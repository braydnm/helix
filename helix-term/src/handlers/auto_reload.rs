...
impl AutoReloadHandler {
    ...
    fn handle_external_updates(&mut self) {
        ...
        job::dispatch_blocking_editor_only(move |editor: &mut Editor| {
            ...
            let focus_view_id = editor
                .most_recent_client_id
                .and_then(|cid| editor.clients.get(cid))
                .map(|client| client.tree.focus);
            ...
            for (doc_id, update) in updated_doc_ids {
                ...
                let focus_view = focus_view_id.and_then(|id| editor.views.try_get(id));
