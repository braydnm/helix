...
use helix_view::{
    document::SavePoint, handlers::completion::ResponseContext, ClientId, Document, Editor, ViewId,
};
...
pub(super) fn completion(
    ...
    if !doc!(editor, client_id).word_completion_enabled() {
        return None;
    }
    let config = editor.config().word_completion;
    let doc_config = doc!(editor, client_id)
        .language_config()
