...
use helix_view::{
    ...
    Align, ClientId, Document, DocumentId, Editor,
};
...
pub fn syntax_symbol_picker(cx: &mut Context) {
    ...
    let picker = Picker::new(
        ...
        (),
        move |cx, tag, action| {
            cx.editor.switch(cx.client_id, doc_id, action);
...
pub fn syntax_workspace_symbol_picker(cx: &mut Context) {
    ...
    let search_root = if let Some(path) = doc!(cx.editor, cx.client_id).path() {
        helix_loader::find_workspace_in(path).0
    } else {
        helix_loader::find_workspace(&helix_stdx::env::current_working_dir()).0
    };
